...
class TestSoftDeleteAuthor:
    ...
    @pytest.mark.asyncio
    async def test_soft_delete_author_success(self, client, create_author, create_user, auth_header)->None:
        ...
        response = await client.delete(f"/authors/{author.id}", headers=header)
    ...
    @pytest.mark.asyncio
    async def test_soft_delete_nonexistent_author(self, client, superadmin_user, auth_header)->None:
        """Попытка удалить несуществующего автора"""
        header = await auth_header(superadmin_user)

        response = await client.delete("/authors/99999", headers=header)
        assert response.status_code == status.HTTP_404_NOT_FOUND

    @pytest.mark.asyncio
    async def test_soft_delete_already_deleted_author(self, client, create_author, superadmin_user, auth_header)->None:
