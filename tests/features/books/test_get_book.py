...
class TestGetBook:
    ...
    def test_get_book(self, client, create_book, create_author, create_user, auth_header)->None:
        """Получить книгу"""
        ...
        response = client.get(f"/books/{book.id}", headers=header)
    ...
    def test_get_book_no_exist(self, client, superadmin_headers)->None:
        """Получить не существующею книгу"""
        response = client.get("/books/1", headers=superadmin_headers)
        assert response.status_code == status.HTTP_404_NOT_FOUND
        data = response.json()

        assert data["detail"] == "Book with id 1 not found"

    def test_get_book_as_deleted(self, client, create_book, superadmin_headers)->None:
        """Получить удаленной книги"""

        book = create_book(deleted_at=datetime.now())

        response = client.get(f"/books/{book.id}", headers=superadmin_headers)
        assert response.status_code == status.HTTP_404_NOT_FOUND
        data = response.json()

        assert data["detail"] == f"Book with id {book.id} not found"

    def test_not_perm(self, client, create_book, create_author, create_user, auth_header)->None:
    ...
    def test_not_auth(self, client, create_book, create_author)->None:
        author = create_author()
        book = create_book(author=author)
